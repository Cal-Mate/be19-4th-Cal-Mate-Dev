...
** 이 프로젝트는 AI 기반 개인 맞춤형 다이어트 관리 플랫폼으로,
사용자의 신체·식단·감정·건강 데이터를 분석해
최적의 식단·운동·동기부여 요소를 자동 추천하는 서비스를 제공한다.
AI 이미지 생성 기술을 활용해 체지방 변화 예측 사진과
음식 칼로리 분석을 지원하며,
식단 빙고·하루 미션·랭킹·칭호·포인트 가챠 등
게임화된 시스템을 통해 지루하지 않게 다이어트를 지속할 수 있다.
또한 감정 기반 음식 추천, 건강 조건 반영 식단 계획, 명절 및 폭식 대비 식단 전략 등
실제 생활 패턴을 반영한 초개인화 기능을 갖추고 있다. **
    ...
    stages {
        ...
        stage('Source Build') {
            steps {
                ...
                bat '''                    xcopy /B /Y "C:\\workspace\\be14-4th-clover-moodiary-BE\\moodiary\\src\\main\\resources\\application.yml" "moodiary\\src\\main\\resources\\application.yml"
                '''
                script {
                    dir(path: 'moodiary') {
                        if (isUnix()) {
